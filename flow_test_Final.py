...
sns.scatterplot(data=pca_df, x='Componente Principal 1', y='Componente Principal 2', hue='Classe', palette='plasma', s=80) #
...
fig = plt.figure(figsize=(10, 8))
ax = fig.add_subplot(111, projection='3d')
scatter = ax.scatter(pca_df['Componente Principal 1'], pca_df['Componente Principal 2'], pca_df['Componente Principal 3'], c=pca_df['Classe'].astype('category').cat.codes, cmap='plasma', s=80)
...
# Adicionar colorbar
cbar = plt.colorbar(scatter, ax=ax, ticks=np.unique(pca_df['Classe'].astype('category').cat.codes))
...
modelos_parametros = [
    ...
    (RandomForestClassifier(random_state=42), {'n_estimators': [50, 100, 200], 'max_depth': [None, 10, 20, 30],
...
df_accuracy = pd.DataFrame(accuracy_values)
...
sns.boxplot(x='Model', y='Accuracy', data=df_accuracy )
